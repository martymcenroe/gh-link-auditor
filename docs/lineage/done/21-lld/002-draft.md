...
|------|-------------|-------------|
| `slant/__init__.py` | Add | Package init with version and exports |
| `slant/__main__.py` | Add | CLI entry point for `python -m slant` |
| `slant/cli.py` | Add | Argument parsing for `score` and `dashboard` subcommands |
| `slant/config.py` | Add | Configurable signal weights with defaults |
| `slant/scorer.py` | Add | Main scoring engine: loads report, iterates candidates, computes verdicts |
| `slant/models.py` | Add | Verdict dataclass/TypedDict definitions |
| `slant/signals/__init__.py` | Add | Signals subpackage init |
| `slant/signals/redirect.py` | Add | HTTP redirect detection signal |
| `slant/signals/title.py` | Add | Title fuzzy match signal |
| `slant/signals/content.py` | Add | Content similarity signal |
| `slant/signals/url_path.py` | Add | URL path similarity signal |
| `slant/signals/domain.py` | Add | Domain match signal |
| `slant/dashboard.py` | Add | HITL dashboard HTTP server with inline HTML/CSS/JS |
...
| `docs/reports/21/implementation-report.md` | Add | Implementation report |
| `docs/reports/21/test-report.md` | Add | Test report |
...
- All "Add" files have valid parent directories (`slant/`, `slant/signals/`, `tests/unit/`, `tests/fixtures/`, `docs/reports/21/`)
...
def write_verdicts(verdicts: VerdictsFile, output_path: Path) -> None:
    """Write verdicts to JSON file with validation."""
    ...

# slant/signals/redirect.py
def check_redirect(dead_url: str, candidate_url: str, timeout: float = 10.0) -> float:
    """Check if dead URL redirects to candidate. Returns 0.0-1.0 ratio."""
    ...

# slant/signals/title.py
...
# slant/signals/content.py
def compare_content(candidate_url: str, archived_content: str, timeout: float = 10.0) -> float:
    """Fetch candidate, strip HTML, compare to archived content. Returns 0.0-1.0 ratio."""
    ...
...
| T080 | Dashboard starts and responds to GET / | Start server, GET /, verify 200 | RED |
| T090 | Dashboard HTML contains two iframes | Parse response, find archive and candidate iframes | RED |
| T100 | POST /api/decide updates verdict file | POST approval, verify file changed | RED |
| T110 | POST /api/decide rejects invalid decision | POST "maybe", verify 400 response | RED |
| T120 | Keyboard shortcut 'a' triggers approve | Selenium/manual test of JavaScript | RED |
...
|----|----------|------|-------|-----------------|---------------|
| 010 | Score report produces correct verdict count | Auto | Fixture with 5 dead links | 5 verdicts | len(verdicts) == 5 |
| 020 | Verdict has all required fields | Auto | Any verdict | All 7 fields present | All keys exist |
| 030 | Redirect 301 to candidate scores ≥40 | Auto | Mock 301 redirect | redirect score ≥40 | breakdown.redirect >= 40 |
| 040 | Identical title scores ~25 | Auto | Same title | title_match ~25 | 24 ≤ score ≤ 25 |
| 050 | Score 95 maps to AUTO-APPROVE | Auto | Composite 95 | "AUTO-APPROVE" | verdict == "AUTO-APPROVE" |
| 060 | Score 87 maps to HUMAN-REVIEW | Auto | Composite 87 | "HUMAN-REVIEW" | verdict == "HUMAN-REVIEW" |
| 070 | Score 62 maps to LOW-CONFIDENCE | Auto | Composite 62 | "LOW-CONFIDENCE" | verdict == "LOW-CONFIDENCE" |
| 080 | Score 31 maps to INSUFFICIENT | Auto | Composite 31 | "INSUFFICIENT" | verdict == "INSUFFICIENT" |
| 090 | AUTO-APPROVE sets auto decision | Auto | Score 95+ | human_decision="auto" | field set correctly |
| 100 | Zero candidates = INSUFFICIENT | Auto | Empty candidates | INSUFFICIENT, confidence=0 | Both conditions met |
| 110 | Dashboard GET / returns 200 | Auto | Server started | HTTP 200 | response.status == 200 |
| 120 | Dashboard HTML has two iframes | Auto | GET / | HTML with 2 iframes | iframe count == 2 |
| 130 | POST /api/decide with "approved" succeeds | Auto | Valid decision | HTTP 200, file updated | Status 200, file changed |
| 140 | POST /api/decide with "maybe" returns 400 | Auto | Invalid decision | HTTP 400 | Status 400 |
| 150 | POST /api/decide with malformed JSON returns 400 | Auto | "{invalid" | HTTP 400 | Status 400 |
| 160 | Redirect timeout scores 0 | Auto | Mock timeout | redirect score = 0 | breakdown.redirect == 0 |
| 170 | Content fetch error scores 0 | Auto | Mock connection error | content score = 0 | breakdown.content == 0 |
| 180 | URL path similarity computed correctly | Auto | /blog/post-1 vs /blog/post-1 | 1.0 ratio | score == 10 |
| 190 | Domain exact match scores 5 | Auto | Same domain | domain score = 5 | breakdown.domain == 5 |
| 200 | Domain mismatch scores 0 | Auto | Different domains | domain score = 0 | breakdown.domain == 0 |
| 210 | Dashboard summary shows when all decided | Auto | All verdicts decided | Summary HTML | "Summary" in response |
| 220 | CLI --help works | Auto | --help flag | Help text | Exit 0, contains "usage" |
...
# Run with coverage
poetry run pytest tests/unit/test_scorer.py tests/unit/test_signals.py tests/unit/test_dashboard.py -v --cov=slant --cov-report=term-missing
...
|----|----------|-------------------|-------|
| M010 | Keyboard shortcuts in browser | JavaScript interaction requires browser automation | 1. Start dashboard 2. Open browser 3. Press 'a' 4. Verify approve sent |
| M020 | Iframe fallback on X-Frame-Options | Requires real page with X-Frame-Options | 1. Add URL with X-Frame-Options to fixture 2. Open dashboard 3. Verify fallback shown |

*Note: M010 could be automated with Playwright in future. M020 requires mock server setup.*
...
### Code
- [ ] All modules implemented (`slant/scorer.py`, signals, dashboard, CLI)
...
- [ ] Test coverage ≥95% for `slant/` package
...
- `slant/scorer.py` ✓ (in Section 2.1)
- `slant/dashboard.py` ✓ (in Section 2.1)
- `slant/cli.py` ✓ (in Section 2.1)
