...
Update Reason: New LLD for automated link checking pipeline
-->
...
|------|-------------|-------------|
| `pyproject.toml` | Add | Poetry project configuration with dependencies |
| `README.md` | Add | Project documentation and usage instructions |
| `link_check_pipeline/__init__.py` | Add | Package initialization |
| `link_check_pipeline/link_checker.py` | Add | Core link extraction and validation logic |
| `link_check_pipeline/policy_discovery.py` | Add | CONTRIBUTING.md parser for commit standards |
| `link_check_pipeline/repository_manager.py` | Add | Fork, clone, branch, and PR submission |
| `link_check_pipeline/iteration_engine.py` | Add | Orchestrates workflow across target repos |
| `link_check_pipeline/database.py` | Add | SQLite tracking for PR status and metrics |
| `link_check_pipeline/metrics.py` | Add | Reporting and analytics generation |
| `link_check_pipeline/config.py` | Add | Configuration management and constants |
| `link_check_pipeline/cli.py` | Add | CLI entry point using Click |
| `tests/__init__.py` | Add | Test package initialization |
| `tests/unit/__init__.py` | Add | Unit test package initialization |
...
| `tests/fixtures/` | Add | Test fixtures directory |
...
| `.github/workflows/ci.yml` | Add | GitHub Actions CI configuration |
| `config/default_targets.yaml` | Add | Default target repository list |
...
**All files are "Add" type for a new repository - parent directories will be created as part of initialization.**
...
3. FOR each target repository:
   a. Check if recently processed (skip if within cooldown)
...
- Orphaned forks can be cleaned via `cleanup` command
...
|----|----------|------|-------|-----------------|---------------|
| 010 | Extract links from markdown | Auto | Markdown with links | List of (url, line) tuples | All links found |
| 020 | Skip code block links | Auto | Markdown with fenced code | Empty list | No false positives |
| 030 | Validate working link | Auto-Live | httpbin.org/status/200 | VALID status | Correct classification |
| 040 | Validate broken link | Auto-Live | httpbin.org/status/404 | BROKEN status | Correct classification |
| 050 | Handle timeout | Auto | Mocked slow response | TIMEOUT status | Returns within budget |
| 060 | Parse conventional commits | Auto | Sample CONTRIBUTING.md | PolicyConfig with prefix | Correct extraction |
| 070 | Parse Angular-style commits | Auto | Angular CONTRIBUTING.md | PolicyConfig with scope | Correct extraction |
| 080 | Handle missing CONTRIBUTING | Auto | Empty response | Default PolicyConfig | Graceful fallback |
| 090 | Format commit per policy | Auto | PolicyConfig + counts | Formatted message | Matches template |
| 100 | Initialize database | Auto | New DB path | Tables exist | Schema correct |
| 110 | Record PR | Auto | PRRecord dict | Inserted row | ID returned |
| 120 | Update PR status | Auto | PR ID + new status | Updated row | Status changed |
| 130 | Calculate acceptance rate | Auto | Sample PR data | Float percentage | Correct math |
| 140 | End-to-end dry run | Auto | Test repo path | No PR created, report generated | Full flow executes |
...
# Run with coverage
poetry run pytest tests/ -v --cov=link_check_pipeline --cov-report=html
```
...
| Fork accumulation | Low | High | Periodic cleanup command, auto-delete merged PR forks |
...
- Risk: Rate limit exhaustion → Mitigated by: exponential backoff in `validate_link()`
- Risk: Policy parsing failures → Mitigated by: fallback in `parse_commit_conventions()`
- Risk: Fork accumulation → Mitigated by: cleanup logic (not yet in signatures - add `cleanup_forks()`)
