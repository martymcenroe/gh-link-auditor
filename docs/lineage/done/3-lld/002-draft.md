...
Updated By: Issue #3 initial draft
Update Reason: Initial LLD creation for Repo Scout feature
Previous: N/A
-->
...
poetry run pytest tests/unit/test_repo_scout/ -v

# Run only fast/mocked tests (exclude live)
poetry run pytest tests/unit/test_repo_scout/ -v -m "not live"

# Run live integration tests (requires tokens)
poetry run pytest tests/unit/test_repo_scout/ -v -m live

# Run with coverage
poetry run pytest tests/unit/test_repo_scout/ -v --cov=src/repo_scout --cov-report=term-missing
```
