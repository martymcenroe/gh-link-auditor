...
- [x] **Legal/External Data:** Yes — fetches from Internet Archive (CDX API is public, no ToS issues for automated queries with rate limiting) and GitHub API (public API, rate-limited). Respects `robots.txt` is N/A for API endpoints. Search heuristics use direct URL construction, not scraping search engines. **Data Processing: Local execution only. No external transmission of scraped content.**
