...
Update Reason: New feature design for automated documentation fix bot
-->
...
| `config/targets.yaml` | Add | Repository target list configuration |
| `config/bot_config.yaml` | Add | Bot behavior configuration |
...
| `tests/integration/test_end_to_end.py` | Add | Integration tests for full workflow |
...
1. **R1:** Bot can load and parse 100+ repository targets from YAML configuration
2. **R2:** Link scanner correctly identifies broken links (404, 410, 403 with verification)
3. **R3:** Link scanner handles anti-bot responses (403/405) with appropriate headers
4. **R4:** Bot executes complete 9-step Git workflow automatically
5. **R5:** PRs are created with professional commit messages and descriptions
6. **R6:** State persistence prevents duplicate PR submissions for same broken link
7. **R7:** Daily execution respects rate limits (configurable max PRs/day)
8. **R8:** Bot provides structured reporting (JSON) of scan results
9. **R9:** GitHub Action runs daily on schedule
10. **R10:** All operations are logged with structured logging
...
```
targets.yaml ──load──► TargetRepository[] ──prioritize──► Scan Queue
     │
     ▼
...
- [ ] Test files created at: `tests/unit/test_*.py`, `tests/integration/test_*.py`

### 10.1 Test Scenarios

| ID | Scenario | Type | Input | Expected Output | Pass Criteria |
|----|----------|------|-------|-----------------|---------------|
| 010 | Load valid targets | Auto | Valid YAML file | List[TargetRepository] | Length > 0, all fields valid |
| 020 | Load invalid targets | Auto | Malformed YAML | ConfigError raised | Exception with message |
| 030 | Check working link | Auto | https://example.com | status=200, broken=false | Correct status |
| 040 | Check broken link | Auto | Mock 404 URL | status=404, broken=true | Marked as broken |
| 050 | Handle anti-bot 403 | Auto | Mock 403 URL | Retry with headers | At least 2 requests made |
| 060 | Extract markdown links | Auto | Sample .md content | List of (url, line) | All links found with lines |
| 070 | Generate branch name | Auto | TargetRepo + context | "fix/broken-link-*" | Valid git ref name |
| 080 | Generate commit message | Auto | List[BrokenLink] | "fix: correct broken..." | Conventional format |
| 090 | Generate PR body | Auto | List[BrokenLink] | Markdown PR body | Contains link list |
| 100 | Prevent duplicate PR | Auto | Previously submitted link | was_already_fixed=true | Returns true |
| 110 | Respect daily limit | Auto | Limit=2, 3 broken links | 2 PRs submitted | Exactly 2 |
| 120 | End-to-end workflow | Auto-Live | Real test repo | PR created | PR URL returned |
| 130 | Handle clone failure | Auto | Invalid repo URL | ScanResult with error | Error field populated |
| 140 | Handle network timeout | Auto | Mock timeout URL | Retry then fail | Max 3 retries, then broken |
| 150 | Suggest fix from archive.org | Auto | Known archived URL | Suggested fix URL | Confidence > 0.5 |
