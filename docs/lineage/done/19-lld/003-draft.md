...
Update Reason: Revised to fix mechanical test plan validation - added missing REQ coverage for REQ-1, REQ-2, REQ-4, REQ-13
-->
...
1. Run the #22 pipeline across a list of repositories provided by #3 Repo Scout with configurable concurrency (1–N workers).
2. If a batch run is interrupted, it can be resumed from the last checkpoint without reprocessing completed repos.
3. A failure in any single repo (clone failure, pipeline error, PR submission error) must not affect processing of other repos.
4. The engine must read GitHub `X-RateLimit-*` headers and dynamically adjust request pacing to avoid hitting limits.
5. Support one or more GitHub tokens with automatic rotation to the token with the most remaining quota.
6. Before starting a batch, validate that all tokens have required scopes (`repo`, `public_repo` minimum).
7. Display real-time progress to stderr showing repos processed, fixes found, PRs submitted, errors, and ETA.
8. Monitor disk usage of clones and warn/pause when approaching the configured limit.
9. Delete local clones and optionally prune remote branches and stale forks after processing.
10. Generate a summary report (text and JSON) after each batch run.
11. Track PR acceptance rate, time-to-merge, and rejection reasons across multiple batch runs.
12. Expose all functionality through CLI subcommands (`batch run`, `batch resume`, `batch status`, `batch cleanup`, `metrics report`, `metrics campaign`, `metrics refresh-prs`).
13. Support a `--dry-run` flag that runs the full pipeline but skips PR submission.
...
| T300 | Batch run with configurable concurrency 1 and N | Concurrency parameter controls worker count | RED |
| T310 | Rate limiter reads X-RateLimit headers | update_from_headers parses real header format | RED |
| T320 | Dry-run skips PR but completes pipeline | Full scan/fix pipeline executes, pr_submitted=False | RED |
...
|----|----------|------|-------|-----------------|---------------|
| 010 | Load valid target list (REQ-1) | Auto | `sample_target_list.json` (5 repos) | List of 5 RepoTask objects with PENDING status | All fields populated, status=PENDING |
| 020 | Load invalid target list (REQ-1) | Auto | JSON missing `clone_url` field | `BatchInputError` raised | Error message identifies missing field and line |
| 030 | Sequential batch run with concurrency=1 (REQ-1) | Auto | 3 mock repos, concurrency=1, pipeline returns success | RunReport: 3 scanned, 3 succeeded, 0 failed; sequential execution confirmed | Report counters match; tasks processed one at a time |
| 040 | One repo failure isolation (REQ-3) | Auto | 3 repos, 2nd raises ValueError | RunReport: 3 scanned, 2 succeeded, 1 failed | Failed repo has error_message; others unaffected |
| 050 | Checkpoint and resume (REQ-2) | Auto | 5 repos, crash after 3; resume | Only repos 4–5 processed on resume | No duplicate processing; final report covers all 5 |
| 060 | Atomic checkpoint write (REQ-2) | Auto | Save checkpoint, simulate crash mid-write | Previous checkpoint intact | No partial/corrupt checkpoint on disk |
| 070 | Backpressure at low watermark (REQ-4) | Auto | remaining=50, low_watermark=100 | acquire() sleeps >0 seconds | Sleep duration > 0; proportional to distance from reset |
| 080 | No backpressure above high watermark (REQ-4) | Auto | remaining=2000, high_watermark=1000 | acquire() returns immediately | Elapsed time < 10ms |
| 090 | Token rotation (REQ-5) | Auto | 3 tokens with remaining=[100, 500, 200] | get_best_token() returns token with 500 | Correct token returned |
| 100 | Insufficient token scopes (REQ-6) | Auto | Token with scopes=["read:user"] | `InsufficientScopesError` raised | Error message lists missing scopes |
| 110 | All tokens exhausted (REQ-5) | Auto | All tokens remaining=0, reset in future | `AllTokensExhaustedError` with wait_time | wait_time > 0 |
| 120 | Token invalidation (REQ-5) | Auto | Call invalidate_token(); then get_best_token() | Invalidated token excluded | Only valid tokens considered |
| 130 | Progress display format (REQ-7) | Auto | 347/2000 processed, 12 fixes, 3 PRs, 2 errors | `"347/2000 \| 12 fixes \| 3 PRs \| 2 errors \| ETA ..."` | Pattern matches regex |
| 140 | Disk over limit (REQ-8) | Auto | Directory with 11GB, limit=10GB | `(11.0, True)` | Boolean True; usage approximate |
| 150 | Disk under limit (REQ-8) | Auto | Directory with 5GB, limit=10GB | `(5.0, False)` | Boolean False |
| 160 | Cleanup clone (REQ-9) | Auto | Temp directory with files | Directory deleted | `Path.exists()` returns False |
| 170 | Cleanup remote branch (REQ-9) | Auto | Mock GitHub API DELETE | API called with owner/repo/branch | Mock verified with correct URL |
| 180 | RunReport generation (REQ-10) | Auto | BatchState with mixed results | Correct totals for all counters | Every counter independently verified |
| 190 | Campaign metrics aggregation (REQ-11) | Auto | 3 RunReports + 5 PROutcomes in DB | acceptance_rate, avg_time_to_merge correct | Float values within 0.01 tolerance |
| 200 | Text report formatting (REQ-10) | Auto | RunReport with known values | Multi-line text with headers and values | All fields present in output |
| 210 | JSON report formatting (REQ-10) | Auto | RunReport with known values | Valid JSON string | `json.loads()` succeeds; all fields present |
| 220 | Dry-run mode skips PR submission (REQ-13) | Auto | 2 repos, dry_run=True | Pipeline runs; task.pr_submitted=False for all | No PR submission mock called |
| 230 | Per-repo timeout (REQ-3) | Auto | Mock pipeline sleeps 10s, timeout=2s | Task status=FAILED, error="timeout" | Completes in <3s; error message contains "timeout" |
| 240 | Concurrent execution (REQ-1) | Auto | 5 repos, concurrency=3 | All 5 COMPLETED; max 3 concurrent at any time | Semaphore enforced (tracked via mock) |
| 250 | Repo name injection (REQ-3) | Auto | `"../../../etc/passwd"` as repo name | `BatchInputError` raised | Regex validation rejects; no path traversal |
| 260 | Token file permissions (REQ-6) | Auto | File with 0644 permissions | Warning logged | Log output contains permission warning |
| 270 | Stale fork pruning (REQ-9) | Auto | 3 forks: 1 merged (old), 1 open (recent), 1 rejected (old) | Pruning targets: merged-old, rejected-old | 2 forks identified; open fork excluded |
| 280 | Batch status display (REQ-12) | Auto | Checkpoint with 3/5 completed | Formatted summary: "3/5 completed, 0 failed" | Output matches expected format |
| 290 | Integration: full mock batch (REQ-1) | Auto | 3 repos, mocked pipeline + API | Checkpoint saved, report generated, metrics recorded | End-to-end flow completes; DB has run record |
| 300 | Configurable concurrency parameter (REQ-1) | Auto | concurrency=1 then concurrency=5 for same repo set | Both complete; concurrency=5 has higher parallelism in trace | Worker count matches config; all repos processed |
| 310 | Rate limiter parses X-RateLimit headers (REQ-4) | Auto | Headers: `{"X-RateLimit-Remaining": "42", "X-RateLimit-Reset": "1700000000"}` | Internal state updated to remaining=42, reset_at=timestamp | snapshot() reflects parsed values |
| 320 | Dry-run completes full pipeline without PR (REQ-13) | Auto | 1 repo with broken links, dry_run=True | fixes_generated > 0, pr_submitted=False, pr_url=None | Pipeline scan/fix ran; PR submission function never called |
...
|----|---------|--------------|
| M1.1 | "Requirement REQ-1 has no test coverage" | YES - Added (REQ-1) suffix to scenarios 010, 020, 030, 240, 290; added new scenario 300 |
| M1.2 | "Requirement REQ-2 has no test coverage" | YES - Added (REQ-2) suffix to scenarios 050, 060 |
| M1.3 | "Requirement REQ-4 has no test coverage" | YES - Added (REQ-4) suffix to scenarios 070, 080; added new scenario 310 |
| M1.4 | "Requirement REQ-13 has no test coverage" | YES - Added (REQ-13) suffix to scenario 220; added new scenario 320 |
| M1.5 | "Section 3 must use numbered list format" | YES - Converted from bold-prefixed list to plain numbered list |
| M1.6 | "Section 10.1 must reference requirements using (REQ-N) suffix" | YES - Added (REQ-N) suffix to all 32 test scenarios |

### Review Summary

| Review | Date | Verdict | Key Issue |
|--------|------|---------|-----------|
| Mechanical Validator #1 | — | FEEDBACK | 4 requirements had no test coverage; format issues in Sections 3 and 10.1 |
