...
4. PR body includes: count of links fixed, list of affected files, attribution line, and uses the target repo's PR template if present
5. N7 writes the run to campaign DB (`campaign_runs`, `campaign_prs`, `campaign_repos`)
6. Pipeline outputs the PR URL and exits with code 0
...
5. File is written to current directory; user opens it in a browser
...
2. Served locally or deployed to GitHub Pages
3. `--refresh` flag queries GitHub API for current PR statuses before regeneration
...
- **PR Automation (N6):** Uses `PyGithub` (primary) or `gh` CLI (fallback) for fork → branch → commit → push → PR workflow
