...
Update Reason: LLD for Issue #5 - State Database implementation
-->
...
| `src/gh_link_auditor/__init__.py` | Modify | Export new database components |
...
|----|----------|------|-------|-----------------|---------------|
| 010 | Create new database | Auto | Empty path | Tables created | Schema matches spec |
| 020 | Record new interaction | Auto | Valid interaction data | Record ID returned | Record retrievable |
| 030 | Detect submitted URL | Auto | Existing repo+url | True | No false negatives |
| 040 | Allow new URL | Auto | New repo+url | False | No false positives |
| 050 | Update status to merged | Auto | Record ID + MERGED | Status updated | updated_at changed |
| 060 | Add repo to blacklist | Auto | Repo URL | Entry ID returned | is_blacklisted returns True |
| 070 | Block blacklisted repo | Auto | Blacklisted repo | can_submit_fix False | Reason includes "blacklisted" |
| 080 | Block blacklisted maintainer | Auto | Blacklisted maintainer | can_submit_fix False | Reason includes "blacklisted" |
| 090 | Allow clean submission | Auto | New repo, no blacklist | can_submit_fix True | Reason is "ok" |
| 100 | Block duplicate submission | Auto | Same repo+url twice | Second can_submit_fix False | Reason includes "already" |
| 110 | Handle expired blacklist | Auto | Expired entry | is_blacklisted False | Entry ignored |
| 120 | Get statistics | Auto | Populated DB | Correct counts | Matches manual count |
| 130 | Close and reopen | Auto | DB path | Data persisted | Records still present |
