...
Update Reason: Initial LLD creation for Issue #22
-->
...
|------|-------------|-------------|
| `src/ghla/pipeline/__init__.py` | Add | Pipeline package init with public exports |
| `src/ghla/pipeline/state.py` | Add | PipelineState TypedDict and state utilities |
| `src/ghla/pipeline/graph.py` | Add | LangGraph StateGraph wiring and conditional edges |
| `src/ghla/pipeline/nodes/__init__.py` | Add | Nodes package init |
| `src/ghla/pipeline/nodes/n0_load_target.py` | Add | Target loading node (repo URL or local path) |
| `src/ghla/pipeline/nodes/n1_scan.py` | Add | Lu-Tze scan wrapper node |
| `src/ghla/pipeline/nodes/n2_investigate.py` | Add | Cheery investigation node |
| `src/ghla/pipeline/nodes/n3_judge.py` | Add | Mr. Slant judging node |
| `src/ghla/pipeline/nodes/n4_human_review.py` | Add | Terminal HITL review node |
| `src/ghla/pipeline/nodes/n5_generate_fix.py` | Add | Fix generation node |
| `src/ghla/pipeline/cost_tracker.py` | Add | LLM cost accumulator and circuit breaker |
| `src/ghla/pipeline/circuit_breaker.py` | Add | Circuit breaker logic for max_links threshold |
| `src/ghla/cli/__init__.py` | Add | CLI package init |
| `src/ghla/cli/run.py` | Add | `ghla run` CLI command implementation |
| `src/ghla/cli/main.py` | Modify | Register `run` command in CLI group |
| `src/ghla/db/models.py` | Modify | Add pipeline run tracking tables |
...
| `tests/fixtures/pipeline/` | Add | Static fixture directory for pipeline tests |
...
|---------|--------|-------|
| `fixtures/pipeline/scan_results_all_high.json` | Generated | 8 dead links, all with high-confidence candidates |
| `fixtures/pipeline/scan_results_mixed.json` | Generated | 12 dead links, 9 high + 3 low confidence |
| `fixtures/pipeline/scan_results_over_limit.json` | Generated | 200 dead links (circuit breaker trigger) |
| `fixtures/pipeline/scan_results_empty.json` | Generated | 0 dead links |
| `fixtures/pipeline/mock_llm_responses/` | Generated | Pre-recorded LLM responses for deterministic testing |
| `fixtures/pipeline/sample_repo/` | Generated | Minimal repo with documentation containing known dead links |
...
|----|----------|------|-------|-----------------|---------------|
| 010 | N0 validates GitHub URL | Auto | `https://github.com/org/repo` | target_type="url", repo_name="org/repo" | Extracted correctly |
| 020 | N0 validates local path | Auto | `/tmp/test-repo` (existing) | target_type="local", repo_name="test-repo" | Extracted correctly |
| 030 | N0 rejects invalid URL | Auto | `not-a-url` | ValueError raised | Exception type matches |
| 040 | N0 rejects missing path | Auto | `/nonexistent/path` | FileNotFoundError raised | Exception type matches |
| 050 | N0 lists doc files only | Auto | Repo with .md, .py, .rst | Only .md, .rst returned | Extension filter works |
| 060 | N1 parses scan JSON | Auto | Issue #8 schema JSON | List[DeadLink] | Schema compliance |
| 070 | N1 handles clean repo | Auto | Repo with no dead links | dead_links=[] | Empty list returned |
| 080 | Circuit breaker triggers | Auto | 51 dead links, max=50 | triggered=True | Exit code 2 |
| 090 | Circuit breaker passes | Auto | 50 dead links, max=50 | triggered=False | Continues to N2 |
| 100 | N2 Wayback success | Auto | Archived URL | ReplacementCandidate with source="wayback" | Candidate found |
| 110 | N2 no candidates | Auto | URL with no archive | candidates=[] | Empty list, no error |
| 120 | N3 high confidence | Auto | Exact title match candidate | confidence >= 0.8 | Threshold passed |
| 130 | N3 low confidence | Auto | Unrelated candidate | confidence < 0.8 | Routes to N4 |
| 140 | N4 receives low confidence | Auto | Mixed verdicts | Only low-conf in N4 | Correct routing |
| 150 | N4 skipped if all high | Auto | All verdicts >= 0.8 | N4 not invoked | Skip confirmed |
| 160 | N5 valid diff | Auto | Approved verdict | Unified diff applies cleanly | `patch --dry-run` succeeds |
| 170 | Cost accumulation | Auto | 3 LLM calls | total = sum(costs) | Math correct |
| 180 | Cost limit halt | Auto | max_cost=0.01 | Exit code 3 | Pipeline halts |
| 190 | Dry run behavior | Auto | --dry-run flag | N4/N5 not executed | JSON output only |
| 200 | Full pipeline success | Auto | Fixture repo | Exit code 0, patches written | E2E passes |
| 210 | Network retry | Auto | Mock 2 failures then success | Request succeeds | 3 retries work |
| 220 | Network exhaustion | Auto | Mock 4 failures | Exit code 1, partial saved | Graceful failure |
| 230 | State persistence | Auto | Run to N3, check DB | All nodes' state in DB | Data integrity |
| 240 | Human review approve | Auto | Piped `y` input | verdict.approved=True | Input parsing |
| 250 | Human review reject | Auto | Piped `n` input | verdict.approved=False | Input parsing |
...
# Run with coverage
poetry run pytest tests/unit/pipeline/ tests/integration/pipeline/ --cov=src/ghla/pipeline --cov-report=html
