...
Update Reason: Initial LLD creation for logging configuration
-->
...
- [ ] What log retention policy should be applied to file logs (days, size, rotation)?
- [ ] Should log files be added to .gitignore?
...
|---------|------------------|-------------------|--------|
| T010 | test_setup_logging_returns_logger | Returns configured Logger instance | RED |
| T020 | test_console_handler_added | StreamHandler attached when console=True | RED |
| T030 | test_file_handler_added | RotatingFileHandler attached when file=True | RED |
| T040 | test_log_directory_created | logs/ directory created if missing | RED |
| T050 | test_log_level_configurable | Logger level matches parameter | RED |
| T060 | test_no_duplicate_handlers | Calling setup twice doesn't duplicate handlers | RED |
| T070 | test_get_logger_returns_same | get_logger returns previously configured logger | RED |
| T080 | test_log_format_includes_timestamp | Log output includes ISO timestamp | RED |
| T090 | test_check_links_uses_logging | check_links.py logs instead of prints | RED |
...
|----|----------|------|-------|-----------------|---------------|
| 010 | Logger creation | Auto | name="test" | Logger instance | isinstance(result, logging.Logger) |
| 020 | Console handler | Auto | console=True | StreamHandler in handlers | Any handler is StreamHandler |
| 030 | File handler | Auto | file=True | RotatingFileHandler in handlers | Any handler is RotatingFileHandler |
| 040 | Directory creation | Auto | log_dir="new_logs" | Directory exists | Path("new_logs").exists() |
| 050 | Level configuration | Auto | level="DEBUG" | Logger.level == DEBUG | logger.level == logging.DEBUG |
| 060 | Idempotent setup | Auto | Call setup twice | Single set of handlers | len(handlers) == expected count |
| 070 | Logger retrieval | Auto | setup then get | Same logger | logger1 is logger2 |
| 080 | Format validation | Auto | Log a message | Contains timestamp | Regex matches ISO format |
| 090 | check_links integration | Auto | Run find_urls | No print output, log records exist | caplog has records |
