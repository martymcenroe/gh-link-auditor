...
Update Reason: Fixed SSRF vulnerability, worktree scope violation, and destructive action confirmation issues
Previous: Fixed Test ID mismatch between Section 10.0 and 10.1, resolved open questions, added blocklist config
-->
...
*All questions resolved in Gemini Review #1:*

- [x] What is the target repository list source? **RESOLVED: Use `data/default_targets.yaml` as MVP source. Future iterations can integrate GitHub Topics API.**
- [x] Should we include a blocklist mechanism for repositories that reject PRs or have opted out? **RESOLVED: Yes. Added `data/blocklist.yaml` to prevent checking/PRing opted-out repositories.**
- [x] What is the acceptable rate limit strategy for GitHub API calls? **RESOLVED: Authenticated requests via `gh` token are mandatory. Exponential backoff strategy in Section 2.5.**
- [x] Should dead link replacement suggest alternatives or simply remove the link? **RESOLVED: Remove only. Suggesting alternatives introduces hallucination risk.**
...
| `src/gh_link_auditor/url_validator.py` | Add | SSRF protection and URL validation |
...
| `tests/unit/test_metrics.py` | Add | Unit tests for metrics calculations |
| `tests/unit/test_iteration_engine.py` | Add | Unit tests for iteration engine |
...
| `tests/fixtures/sample_contributing_angular.md` | Add | Angular-style CONTRIBUTING.md |
...
| `data/blocklist.yaml` | Add | Blocklist for opted-out repositories |
...
def validate_url_safety(url: str) -> LinkResult | None:
    """
    Pre-validation check before HTTP request.
    Returns LinkResult with SSRF_BLOCKED status if unsafe, None if safe to proceed.
    """
    ...
...
async def validate_link(url: str, timeout: float = 10.0) -> LinkResult:
    ...
    SECURITY: Calls validate_url_safety() before making HTTP request.
    """
    ...
...
def get_pipeline_work_dir() -> Path:
    """
    Return the pipeline working directory within project worktree.
    Creates ./.pipeline_work/ if it doesn't exist.
...
def cleanup_forks(
    ...
    Raises SafetyError if confirm_destructive is False.
    """
    ...
...
2. Load blocklist from data/blocklist.yaml
3. Initialize database connection
4. Create pipeline work directory (./.pipeline_work/) within project worktree
5. FOR each target repository:
   a. Check if blocklisted (skip if yes)
   b. Check if recently processed (skip if within cooldown - idempotency)
   c. Fetch CONTRIBUTING.md → parse policy
   d. Fork repository (if not already forked)
   e. Clone to ./.pipeline_work/{repo_name}/ (within worktree)
   f. Extract all links from markdown/text files
   g. Validate links concurrently (rate-limited, SSRF-protected)
   h. IF broken links found:
      i. Create fix branch
      ii. Apply fixes (remove broken links)
      iii. Format commit message per policy
      iv. Commit and push
      v. Create PR via gh CLI (non-interactively)
      vi. Record PR in database
   i. Cleanup temporary directory
6. Generate metrics report (acceptance rate, time-to-merge)
7. Log summary

LINK VALIDATION FLOW (with SSRF protection):
1. Receive URL
2. Parse URL → extract domain
3. IF domain in skip_list THEN skip
4. SSRF CHECK (NEW):
   a. Resolve hostname to IP address(es)
   b. FOR each resolved IP:
      - Check against PRIVATE_IP_RANGES
      - IF private/reserved → return SSRF_BLOCKED status
   c. Block: localhost, 127.x.x.x, 10.x.x.x, 172.16-31.x.x, 192.168.x.x, 169.254.x.x
5. Send HEAD request (fallback to GET)
6. Follow redirects (max 5)
...
7. Classify response:
   ...
   - SSL error → SSL_ERROR
8. Return LinkResult with details
...
CLEANUP FORKS FLOW (with safety confirmation):
1. Check confirm_destructive flag
2. IF confirm_destructive is False:
   - Raise SafetyError("Destructive cleanup requires --confirm-destructive-cleanup flag")
3. List all forks owned by authenticated user
4. FOR each fork:
   a. Check if associated PR was merged
   b. Check if fork age > max_age_days
   c. IF both conditions met → delete fork
5. Return list of deleted repository names
```
...
* **Key Decisions:**
  ...
  - **SSRF protection via DNS resolution before HTTP requests** - security requirement
  - **Worktree-scoped temp directory** - safety requirement, uses `./.pipeline_work/`
  - **Explicit confirmation for destructive operations** - safety requirement
...
| Temp Directory Location | System /tmp, project subdir | ./.pipeline_work/ | Keeps all operations within worktree scope |
| SSRF Protection | Pre-request DNS check, proxy filtering | Pre-request DNS check | Direct control, no external dependencies |
| Destructive Operations | Auto-confirm, manual confirm, flag-required | Flag-required | Prevents accidental deletions |
...
- **All file operations must remain within project worktree**
- **No HTTP requests to private/reserved IP ranges**
...
11. **Link validation blocks requests to private/reserved IP ranges (SSRF protection)**
12. **All temporary files are created within project worktree (./.pipeline_work/)**
13. **Destructive cleanup operations require explicit confirmation flag**
...
| System /tmp for clones | Standard temp location | Violates worktree scope governance | **Rejected** |
| Project subdir for clones | Within worktree, gitignored | Requires cleanup | **Selected** |
| Auto-confirm fork cleanup | Convenient | Risk of accidental deletion | **Rejected** |
| Flag-required fork cleanup | Explicit confirmation | Requires extra flag | **Selected** |

**Rationale:** The combination of asyncio for link checking and gh CLI for PR creation balances performance with operational simplicity. SQLite provides sufficient durability for a single-operator pipeline without external dependencies. Worktree-scoped operations and explicit destructive confirmation address safety governance requirements.
...
GitHub Topics API ──fetch──► Target List (YAML)
                                    │
                                    ▼
Target Repo ──clone──► ./.pipeline_work/{repo}/ ──extract──► Link List
                                                                  │
                                                                  ▼
Link List ──SSRF check──► Safe Links ──validate──► Broken Links ──fix──► Commit ──push──► PR
                                                                                            │
                                                                                            ▼
                                                                                     SQLite DB (tracking)
```
...
| `ssrf_test_urls.json` | Generated | URLs with private IPs for SSRF testing |
...
- Working directory: `./.pipeline_work/` (gitignored)
...
flowchart TD
    ...
    subgraph Phase2["Phase 2: Analysis"]
        D[Fork Repository] --> E[Clone to<br/>.pipeline_work/]
        E --> F[Extract Links]
        F --> G[SSRF Check<br/>Block Private IPs]
        G --> H[Validate Links<br/>async, concurrent]
    end

    subgraph Phase3["Phase 3: Fix & Submit"]
        I{Broken Links?}
        I -->|Yes| J[Apply Fixes]
        J --> K[Format Commit<br/>per Policy]
        K --> L[Push Branch]
        L --> M[Create PR via gh]
        I -->|No| N[Skip Repository]
    end
...
| **SSRF vulnerability** | **Pre-request DNS resolution validates all URLs are not private/reserved IPs (RFC 1918, loopback, link-local). Redirect targets also validated. See `url_validator.py`.** | **Addressed** |
...
| **Worktree scope violation** | **All temp operations use `./.pipeline_work/` within project root, never system `/tmp`. Directory created by `get_pipeline_work_dir()` and gitignored.** | **Addressed** |
| **Accidental fork deletion** | **`cleanup_forks()` requires `--confirm-destructive-cleanup` CLI flag. Raises `SafetyError` without explicit confirmation.** | **Addressed** |
...
- Orphaned forks can be cleaned via `cleanup_forks()` function with explicit confirmation
...
| DNS resolution | < 100ms per URL | Cache DNS results per domain |

**Bottlenecks:** 
- Large repositories with 1000+ links will be slower
- Rate-limited GitHub API (5000 req/hour) limits discovery speed
- DNS resolution adds ~10ms overhead per unique domain (SSRF check)
...
|---------|------------------|-------------------|--------|
| T010 | test_extract_links_from_markdown | Extracts URLs with line numbers from markdown | RED |
...
| T056 | test_validate_link_ssrf_localhost | Returns SSRF_BLOCKED for localhost | RED |
| T057 | test_validate_link_ssrf_private_ip | Returns SSRF_BLOCKED for 192.168.x.x | RED |
| T058 | test_validate_link_ssrf_loopback | Returns SSRF_BLOCKED for 127.0.0.1 | RED |
| T059 | test_validate_link_ssrf_redirect | Blocks redirect to private IP | RED |
| T060 | test_parse_conventional_commits | Extracts "fix:" prefix from CONTRIBUTING | RED |
| T070 | test_parse_angular_style | Extracts "fix(scope):" format | RED |
| T080 | test_parse_no_contributing | Returns defaults when no CONTRIBUTING found | RED |
| T085 | test_policy_discovery_80_percent | 80%+ success rate on sample set | RED |
| T090 | test_format_commit_message | Applies policy to commit message | RED |
| T095 | test_format_commit_message_fallback | Falls back to conventional commits | RED |
| T100 | test_create_pr_non_interactive | Creates PR via gh CLI without prompts | RED |
| T110 | test_database_init | Creates tables correctly | RED |
| T120 | test_record_pr | Inserts PR record | RED |
| T130 | test_update_pr_status | Updates status field | RED |
| T140 | test_calculate_acceptance_rate | Computes correct percentage | RED |
| T145 | test_calculate_time_to_merge | Computes average merge time in hours | RED |
| T150 | test_rate_limit_backoff | Retries with exponential delay | RED |
| T155 | test_rate_limit_recovery | Recovers after 429 then 200 | RED |
| T160 | test_process_100_repos | Handles batch without intervention | RED |
| T165 | test_batch_continues_on_error | Failures logged, not fatal | RED |
| T170 | test_idempotent_skip_processed | Skips recently processed repos | RED |
| T175 | test_rerun_after_cooldown | Processes again after cooldown | RED |
| T180 | test_work_dir_within_worktree | Clone dir is ./.pipeline_work/ | RED |
| T185 | test_cleanup_forks_requires_flag | Raises SafetyError without confirm flag | RED |
| T190 | test_cleanup_forks_with_flag | Deletes forks when confirmed | RED |
...
| 056 | Block localhost SSRF (REQ-11) | Auto | http://localhost/test | SSRF_BLOCKED status | Private IP blocked |
| 057 | Block private IP SSRF (REQ-11) | Auto | http://192.168.1.1/test | SSRF_BLOCKED status | RFC 1918 blocked |
| 058 | Block loopback SSRF (REQ-11) | Auto | http://127.0.0.1/test | SSRF_BLOCKED status | Loopback blocked |
| 059 | Block redirect to private IP (REQ-11) | Auto | URL redirecting to 10.0.0.1 | SSRF_BLOCKED status | Redirect target validated |
...
| 180 | Work dir within worktree (REQ-12) | Auto | Call get_pipeline_work_dir() | Path starts with ./.pipeline_work/ | Worktree scope enforced |
| 185 | Cleanup requires confirmation (REQ-13) | Auto | cleanup_forks(confirm=False) | SafetyError raised | Destructive ops blocked |
| 190 | Cleanup with confirmation (REQ-13) | Auto | cleanup_forks(confirm=True) | Forks deleted | Explicit confirm works |
| 200 | End-to-end dry run | Auto | Test repo path | No PR created, report generated | Full flow executes |

**pytest markers for live tests:**
Tests marked as `Auto-Live` (030, 040) MUST use `@pytest.mark.live` decorator to allow exclusion during fast test runs.
...
# Run SSRF protection tests specifically
...
|------|--------|------------|------------|
| PRs perceived as spam | High | Medium | Clear bot identification, genuine value, respect opt-outs via blocklist |
| GitHub rate limit exhaustion | Medium | Medium | Exponential backoff via `rate_limited_request()`, daily quotas, authenticated requests |
| False positive broken links | Medium | Low | Conservative timeout, retry logic, human review option |
| Repository owner complaints | Medium | Low | Opt-out list (`data/blocklist.yaml`), clear PR descriptions, responsive to feedback |
| Fork accumulation | Low | High | `cleanup_forks()` function with `--confirm-destructive-cleanup` flag, auto-delete merged PR forks |
| Policy parsing failures | Low | Medium | Robust fallback to conventional commits in `parse_commit_conventions()` |
| Dangling text after link removal | Low | Low | Future work: context-aware removal (noted as limitation) |
| **SSRF attacks via malicious links** | **High** | **Medium** | **`validate_url_safety()` blocks all private/reserved IPs before HTTP requests, including redirects** |
| **Worktree scope violation** | **Medium** | **Low** | **`get_pipeline_work_dir()` enforces ./.pipeline_work/ directory, never system /tmp** |
| **Accidental destructive operations** | **High** | **Low** | **`cleanup_forks()` requires explicit `confirm_destructive=True` or CLI flag** |
...
- [ ] SSRF protection implemented in url_validator.py
- [ ] Worktree-scoped temp directory implemented
- [ ] Destructive operation confirmation implemented

### Tests
- [ ] All test scenarios pass
- [ ] Test coverage ≥95%
- [ ] Live integration tests pass against test repository
- [ ] SSRF protection tests pass (T056-T059)
- [ ] Safety tests pass (T180, T185, T190)
...
- Risk: Rate limit exhaustion → Mitigated by: exponential backoff in `rate_limited_request()`
...
- Risk: Repository owner complaints → Mitigated by: `load_blocklist()` and `is_blocklisted()` in iteration_engine.py
- Risk: SSRF attacks → Mitigated by: `validate_url_safety()` and `is_private_ip()` in url_validator.py
- Risk: Worktree scope violation → Mitigated by: `get_pipeline_work_dir()` in repository_manager.py
- Risk: Accidental destructive operations → Mitigated by: `confirm_destructive` parameter in `cleanup_forks()`
...
| ID | Comment | Implemented? |
|----|---------|--------------|
| G1.1 | "Test ID/Description Mismatch: Section 10.0 and 10.1 are out of sync. T100 in 10.0 is 'test_database_init' but ID 100 in 10.1 is 'Create PR non-interactively'" | YES - Realigned Section 10.0 to match 10.1. T100 now test_create_pr_non_interactive, T110 is test_database_init |
| G1.2 | "Ambiguous 'Auto' vs 'Auto-Live': Ensure pytest markers distinguish live tests" | YES - Added note in 10.1 requiring @pytest.mark.live for Auto-Live tests (030, 040) |
| G1.3 | "Blocklist Configuration: Add data/blocklist.yaml to Section 2.1" | YES - Added to Files Changed table |
| G1.4 | "Link Removal Context: Surrounding text might become dangling" | YES - Added as risk in Section 11 with "Future work" note |
| G1.5 | "Open Questions should be marked resolved" | YES - All 4 questions marked [x] with resolution notes |
...
| ID | Comment | Implemented? |
|----|---------|--------------|
| G2.1 | "CRITICAL - SSRF Vulnerability: Link checker validates arbitrary URLs without blocking private/reserved IP ranges (localhost, 127.x.x.x, 169.254.x.x, 192.168.x.x)" | YES - Added `url_validator.py` module with `is_private_ip()`, `resolve_and_validate_url()`, `validate_url_safety()`. Added SSRF_BLOCKED status. Added tests T056-T059. Added REQ-11. |
| G2.2 | "CRITICAL - Worktree Scope Violation: Clone to temporary directory uses system /tmp instead of project worktree" | YES - Added `get_pipeline_work_dir()` returning `./.pipeline_work/`. Updated all clone operations to use this directory. Added test T180. Added REQ-12. |
| G2.3 | "CRITICAL - Destructive Acts Confirmation: cleanup_forks deletes repositories without explicit confirmation" | YES - Added `confirm_destructive` parameter to `cleanup_forks()`. Function raises SafetyError without explicit flag. Added tests T185, T190. Added REQ-13. |
| G2.4 | "Performance: Consider adding domain cache for DNS lookups" | YES - Added note in Performance section about DNS caching per domain |
| G2.5 | "Maintainability: Check gh CLI version compatibility at startup" | PENDING - Future enhancement, not blocking |
...
| Gemini #1 | 2026-02-16 | REVISE | Test ID mismatch between 10.0 and 10.1 |
| Gemini #2 | 2026-02-16 | REVISE | SSRF vulnerability, worktree scope, destructive confirmation |
