...
Update Reason: Fixed mechanical validation error - added missing directory to Files Changed
-->
...
|----|----------|------|-------|-----------------|---------------|
| 010 | Successful HEAD request | Auto | URL returning 200 | status="ok", code=200 | Status and code match |
| 020 | Redirect response | Auto | URL returning 301 | status="ok", code=301 | Redirects treated as success |
| 030 | Not found response | Auto | URL returning 404 | status="error", code=404 | No retry attempted |
| 040 | Server error response | Auto | URL returning 500 | status="error", code=500 | Correct status |
| 050 | HEAD blocked (405) | Auto | URL returning 405 then 200 on GET | status="ok", method="GET" | Fallback to GET works |
| 060 | HEAD blocked (403) | Auto | URL returning 403 then 200 on GET | status="ok", method="GET" | Fallback to GET works |
| 070 | Rate limited (429) | Auto | URL returning 429 twice then 200 | status="ok", retries=2 | Backoff applied |
| 080 | Retry-After honored | Auto | 429 with Retry-After: 5 | Delay ≥ 5 seconds | Header respected |
| 090 | Request timeout | Auto | Simulated timeout | status="timeout" | Timeout detected |
| 100 | Connection reset | Auto | RemoteDisconnected exception | status="disconnected" | Exception handled |
| 110 | DNS failure | Auto | URLError with DNS reason | status="failed", retries=0 | No retry on DNS |
| 120 | Backoff calculation | Auto | attempt=2, base=1.0 | delay in [4.0, 5.0] | Exponential + jitter |
| 130 | Max retries exhausted | Auto | Always 429 | retries=2, status="error" | Stops at max |
| 140 | Custom User-Agent | Auto | Custom UA string | Header contains custom value | UA sent correctly |
| 150 | SSL verification off | Auto | verify_ssl=False | No SSL errors | Context configured |
