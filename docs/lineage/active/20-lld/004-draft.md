...
Update Reason: Fixed mechanical validation - added full test coverage mapping to requirements
-->
...
|---------|------------------|-------------------|--------|
| T010 | test_investigate_returns_forensic_report | Returns ForensicReport with all required fields (REQ-1) | RED |
| T020 | test_archive_hit_extracts_title | archive_title populated from archived HTML (REQ-2) | RED |
| T030 | test_archive_miss_continues | Investigation continues when no archive (REQ-2) | RED |
| T040 | test_redirect_chain_detected | redirect_chain candidate with verified_live (REQ-3) | RED |
| T050 | test_ssrf_blocked_before_connection | urlopen never called for private IPs (REQ-8) | RED |
| T060 | test_github_rename_detected | github_api_redirect candidate produced (REQ-4) | RED |
| T070 | test_url_heuristic_slugification | Candidates include slugified title paths (REQ-5) | RED |
| T080 | test_url_heuristic_max_three | Maximum 3 heuristic candidates returned (REQ-5) | RED |
| T090 | test_no_search_engine_queries | No requests to search engine domains (REQ-5) | RED |
| T100 | test_cache_hit_no_external_calls | Second call uses cache, no HTTP (REQ-7) | RED |
| T110 | test_rate_limit_backoff | 429 response triggers backoff retry (REQ-6) | RED |
| T120 | test_invalid_scheme_rejected | Non-HTTP scheme raises ValueError (REQ-9) | RED |
| T130 | test_candidates_sorted_by_score | Candidates in descending similarity order (REQ-10) | RED |
| T140 | test_no_candidates_returns_empty | Empty candidates array when nothing found (REQ-1) | RED |
...
|----|----------|------|-------|-----------------|---------------|
| 010 | Happy path - redirect found (REQ-1) | Auto | URL with 301 redirect | ForensicReport with redirect_chain candidate | candidate.method == REDIRECT_CHAIN, verified_live == True |
| 020 | Archive hit with title extraction (REQ-2) | Auto | URL with CDX snapshot | archive_title extracted from HTML | archive_title == mocked title value |
| 030 | Archive miss continues investigation (REQ-2) | Auto | URL with no CDX results | Empty archive fields, investigation continues | investigation_log shows "No archive snapshot found" |
| 040 | Redirect chain followed with SSRF protection (REQ-3) | Auto | URL with multiple 301 redirects | Final URL captured, chain logged | chain_log contains all hops |
| 050 | SSRF blocking - private IP (REQ-8) | Auto | Redirect to 127.0.0.1 | Connection blocked before socket opens | urlopen call_count == 0 for blocked URL, log contains "SSRF blocked" |
| 060 | SSRF blocking - 10.x.x.x (REQ-8) | Auto | Redirect to 10.0.0.1 | Connection blocked | Same as 050 |
| 070 | SSRF blocking - 192.168.x.x (REQ-8) | Auto | Redirect to 192.168.1.1 | Connection blocked | Same as 050 |
| 080 | GitHub repo rename (REQ-4) | Auto | github.com/old/repo 404 | github_api_redirect candidate | candidate.similarity == 1.0 |
| 090 | URL heuristic - title slugification (REQ-5) | Auto | Archive title "Installation Guide" | Candidates include /installation-guide paths | At least one candidate with slugified title |
| 100 | URL heuristic - max 3 results (REQ-5) | Auto | 5 live candidate URLs | Only 3 returned | len(candidates) <= 3 |
| 110 | No search engine queries (REQ-5) | Auto | Any dead URL | No requests to google/bing | Mock call log contains no search domains |
| 120 | Backoff algorithm integration (REQ-6) | Auto | 429 then 200 response | Successful after retry | Final response is 200, backoff applied |
| 130 | Cache hit on second call (REQ-7) | Auto | Same URL twice | Second call no external HTTP | Mock call count same after second call |
| 140 | Invalid scheme rejected (REQ-9) | Auto | ftp://example.com | ValueError raised | Exception type is ValueError |
| 150 | Candidates sorted by score (REQ-10) | Auto | Multiple candidates | Descending similarity order | candidates[0].similarity >= candidates[1].similarity |
| 160 | Empty candidates when nothing found (REQ-1) | Auto | URL with no matches | Empty candidates array | len(candidates) == 0, investigation_log not empty |
| 170 | Similarity score computation (REQ-10) | Auto | Two similar texts | Score between 0.0-1.0 | 0.0 <= score <= 1.0 |
| 180 | URL mutation detection (REQ-3) | Auto | URL with /v1/ path | Tests /v2/, /v3/ variants | mutations tested and logged |
...
|--------|------|---------|-----------|
| Mechanical #1 | 2026-02-16 | REJECTED | Test coverage only 30% - missing REQ mappings |
| (pending) | (auto) | (pending) | Revision to add full REQ coverage |
