...
|---------|------------------|-------------------|--------|
| T010 | test_filter_broken_links_extracts_errors | Returns only non-ok status links | RED |
| T020 | test_filter_broken_links_empty_when_all_ok | Returns empty list when all ok | RED |
| T030 | test_validate_url_accepts_https | Returns True for valid https URL | RED |
| T040 | test_validate_url_rejects_invalid | Returns False for malformed URL | RED |
| T050 | test_apply_resolution_sets_fields | Resolution dict matches schema | RED |
| T060 | test_apply_resolution_timestamp_format | ISO 8601 format verified | RED |
| T070 | test_run_hitl_console_no_broken_links | Returns immediately with message | RED |
| T080 | test_navigation_bounds_checking | Index stays within valid range | RED |
| T090 | test_eof_handling | EOFError triggers graceful exit | RED |
| T100 | test_keyboard_interrupt_handling | KeyboardInterrupt saves and exits | RED |
...
|----|----------|------|-------|-----------------|---------------|
| 010 | Filter broken links | Auto | Results with mixed statuses | List of only non-ok links | Length matches expected count |
| 020 | Filter with all OK | Auto | Results with all ok status | Empty list | Length == 0 |
| 030 | Validate HTTPS URL | Auto | "https://example.com" | True | Returns True |
| 040 | Validate invalid URL | Auto | "not-a-url" | False | Returns False |
| 050 | Apply resolution | Auto | Link dict, action="replace" | Resolution field populated | All schema fields present |
| 060 | Resolution timestamp | Auto | Apply resolution | ISO 8601 string | Matches regex pattern |
| 070 | Empty broken links | Auto | All OK results | Early return | Message printed, results unchanged |
| 080 | Navigation bounds | Auto | Simulated n/p commands | Index clamped 0 to max | No IndexError |
| 090 | EOF handling | Auto | Simulated EOFError | Graceful exit | No exception propagation |
| 100 | Keyboard interrupt | Auto | Simulated KeyboardInterrupt | Save and exit | Results contain applied changes |
| 110 | Replace action flow | Auto | Simulated 'r' + URL input | Resolution with new_url | new_url matches input |
| 120 | Remove action | Auto | Simulated 'd' command | Resolution action=remove | new_url is None |
| 130 | Display formatting | Auto | Broken link dict | Formatted output | Contains URL, status, locations |
...
| Lost work on crash | High | Low | Manual save command; prompt on quit |
...
- Lost work on crash → `run_hitl_console()` with save prompts ✓
